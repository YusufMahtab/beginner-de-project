...
get_user_behaviour = "scripts/sql/get_ser_behaviour_metrics.sql"

# remote config
BUCKET_NAME = "bengalis-first-bucket"
...
def run_redshift_external_query(query):
...
user_purchase_to_rs_stage = PythonOperator(
    ...
    op_kwargs={
        "query" : "alter table spectrum.user_purchase_staging add partition(insert_date='{{ ds }}') \
            location 's3://bengalis-first-bucket/user_purchase_stage/{{ ds }}'"
    }
)
